package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-consortium/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-consortium"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/vendors",s.handleList)
    s.mux.HandleFunc("POST /api/vendors",s.handleCreate)
    s.mux.HandleFunc("PUT /api/vendors/{id}",s.handleUpdate)
    s.mux.HandleFunc("DELETE /api/vendors/{id}",s.handleDelete)
