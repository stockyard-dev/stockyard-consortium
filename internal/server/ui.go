...
func (s *Server) dashboard(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html"); w.Write([]byte(dashHTML))
}
...
if(v.annual_spend)h+='<span>$'+(v.annual_spend/100).toFixed(0)+'/yr</span>';
if(v.contract_end)h+='<span>Ends: '+v.contract_end+'</span>';
...
var h='<h2>'+(isEdit?'EDIT':'NEW')+' VENDOR</h2>';
h+='<div class="fr"><label>Company *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
...
else{await fetch(A+'/vendors',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
closeModal();load();}
